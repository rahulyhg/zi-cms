<?php 
	...
	foreach ($post_cat as $pc) {
		array_push($p_cat, $pc->category);
	}
 ?>

<div class="row">
	<form action="<?=base_url('posts/editpost/'.$post->id_post);?>" method="post">
	<div class="col-sm-9">
		<div class="form-group">
			<label for="Title">Title :</label>
			<input type="text" name="title" class="form-control" id="Title" value="<?=$post->title;?>" placeholder="Enter Title Here">
		</div>
		<div class="form-group">
	...
	<div class="col-sm-3">
		<div class="box box-default">
			<div class="box-header with-border"><h4 class="box-title">Categories</h4></div>
			<div class="box-body">
			<?php foreach ($list_cat as $lc): ?>
				<div class="checkbox">
					<label for="<?=$lc->name;?>"><input type="checkbox" name="categories[]" id="<?=$lc->name;?>" value="<?=$lc->id_category;?>" <?=(in_array($lc->id_category, $p_cat)) ? 'checked' : '';?>><?=$lc->name;?></label>
				</div>
			<?php endforeach; ?>
			</div>
		</div>
		<div class="box box-default">
			<div class="box-header with-border"><h4 class="box-title">Publish</h4></div>
			<div class="box-body">
				<div class="form-group">
					<select name="status" id="" class="form-control" required>
						<?php if ($post->status == 1) {
							echo '<option value="">-- Publish or Draft --</option>';
